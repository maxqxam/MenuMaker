...
class Color:
    def __init__(self,red:float,green:float,blue:float,alpha:float=255):
        ...
        if any([(i<0 or i>255) for i in [self.__red,self.__green,self.__blue,self.__alpha]]):
            raise ValueError(Errors.InvalidColorValue)

    def is_valid(self):
        return any([(i<0 or i>255) for i in [self.__red,self.__green,self.__blue,self.__alpha]])
...
class Pos:
    ...
    def transform(self,sum:float=0,mult:float=1,sum_first:bool=False):
        ...
        if not sum_first:
            self.x *= mult ; self.x += sum
            self.y *= mult ; self.y += sum
        else:
            self.x += sum  ; self.x *= mult
            self.y += sum  ; self.y *= mult
...
class Rect:
    ...
    def __str__(self):
        return "[RectObject : ({},{},{},{})]".format(self.__x,self.__y,
                                                     self.__width,self.__height)

    def get_pygame_rect(self): return pg.rect.Rect(self.get_tuple())
    def get_tuple(self): return self.__x,self.__y,self.__width,self.__height
